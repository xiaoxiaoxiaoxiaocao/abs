...
public class StanzaHandler {
    ...
    public void process(String stanza, XMPPPacketReader reader)
        ...
        // If end of stream was requested  客户端下线了
        ...
        } else if ("message".equals(tag)) {//处理消息
            processMessage(doc);
        } else if ("presence".equals(tag)) {//处理在线状态,假如断线后再次上线
            log.debug("presence...");
            processPresence(doc);
        } else if ("iq".equals(tag)) {//处理IQ消息
            log.debug("iq...");
            processIQ(doc);
        } else {
