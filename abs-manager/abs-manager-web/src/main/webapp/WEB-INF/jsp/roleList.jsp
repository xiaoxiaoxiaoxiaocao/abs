...
<body>	
		<table class="easyui-datagrid" id="rolesId" style="width:100%;height:70%"
			data-options="url:'${baseurl}/user/roleList',fitColumns:true,pagination:true,
					striped:true,checkOnSelect:true,singleSelect:true,toolbar:toolbar">
				<thead>
					<tr>
						<th data-options="field:'ck',checkbox:true"></th>
						<th data-options="field:'id',width:5">角色ID</th>
						<th data-options="field:'name',width:10">角色名称</th>
						<th data-options="field:'comment',width:10">角色描述</th>
					</tr>											
				</thead>
	...
	<div id="addWin" class="easyui-window" title="新增角色"
		data-options="minimizable:false,collapsible:false"
		style="width: 50%; height: 60%; padding: 10px;" closed=true>
			...
			<table cellpadding="5" align="center" style="width: 80%; height: 80%">
				<tr>
					...
					<td><input id="nameId" class="easyui-textbox" type="text" name="name"
						data-options="required:true" style="width: 90%; height: 25px"></input>
				...
				<tr>
					<td>描述:</td>
					<td><input id="messageId" class="easyui-textbox" name="message"
						data-options="multiline:true" style="width: 90%; height: 60px"></input>
					</td>
				</tr>
				<tr>
					<td>授权:</td>
					<td class="qxId" width="90%">
	...
	<div id="editWin" class="easyui-window" title="修改角色" style='width: 50%; height: 100%; padding: 10px;'
		data-options="minimizable:false,collapsible:false"
		 closed=true>
		<form id="ef" method="post" action="${baseurl }/user/addRole">
			<table cellpadding="5" align="center" style="width: 80%; height: 80%">
				...
				<tr>
					<td>描述:</td>
					<td><input id="reMessageId" class="easyui-textbox" name="message"
						data-options="multiline:true" style="width: 90%; height: 60px"></input>
					</td>
				</tr>
				<tr>
					<td>授权:</td>
					<td class="reqxId" width="90%">
					</td>
				</tr>
			</table>
			<div style="text-align: center; padding: 5px">
	...
	function submitForm(){
		...
		var pids =[];//定义一个数组      
	       $('input[name="pms"]:checked').each(function(index,item){
	    	  pids.push($(this).val());
	       });
	      	pids = pids.join(',');
			//alert(pids);		
		...
		$.post("${baseurl }/user/addRole",{"name":name,"message":message,"pids":pids},function(data){
			if(data.status==200){
				$.messager.alert("提示","角色创建成功！","info",function(){
					$("#addWin").window('close');
	...
	function reSubmitForm(){
		var pids =[];//定义一个数组      
	       $('input[name="repms"]:checked').each(function(index,item){
	    	  pids.push($(this).val());
	       });
	      	pids = pids.join(',');
		...
		var message=$("#reMessageId").val();
		if(name.trim().length==0){
			return false;
		}
		$.post("${baseurl }/user/changeRole",{"name":name,"message":message,"id":id,"pids":pids},function(data){
			if(data.status==200){
	...
	*/
	...
	var toolbar = [ {
		...
		handler:function(){
			...
			if(ids.length==0){
				$.messager.alert("错误","至少要选择一个角色","error");
				return;
			}
			$.messager.confirm('确认','确定要删除吗？',function(r){
				if(r){
					$.post("${baseurl}/user/deleteRoleById",{"ids":ids},function(data){
						if(data.status==200)
		...
		handler : function() {
			var id= getSelectIds();
			if (id.length == 0) {
				$.messager.alert('错误', '请选择要修改的角色!','error');
				return;
			}
			
			var row = $("#rolesId").datagrid('getSelected');
			
			$.post("${baseurl}/user/findPermissionsByRoleId",{"roleId":row.id},function(data){
				//$("[name=repms]").attr("checked", true);
				$("[name=repms]").each(function(){
					/* if($(this).val()in data){
					} */
			...
			$("#reMessageId").textbox('setValue',row.comment); 
